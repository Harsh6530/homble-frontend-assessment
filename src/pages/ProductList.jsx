...
const ProductList = () => {
  ...
  if (loading) return <Skeleton width="300px" height="200px" />; // Adjust width and height as needed
  if (error) {
        ...
        {sortedProducts.map((product) => (
          <div key={product.id} className="product-tile">
            <Link to={`/products/${product.id}`}>
              {product.productImage && <img src={product.productImage} class="product-tile img" alt={product.name} />}
              <h2>{product.name}</h2>
